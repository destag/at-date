...
class AtDateTransformer(Transformer):
    ...
    def _inc_number(self, matches):
        ...
        inc_period = (
            matches[1] if matches[1].endswith("s") else matches[1] + "s"
        )
        return datetime(**self.datetime_params) + timedelta(
            **{inc_period: inc_number}
        )

    def _check_if_next_day(self, hour, minute):
        return int(
            self.now.hour > hour
            or (self.now.hour == hour and self.now.minute > minute)
        )

    def _check_if_next_year(self, month, day):
        return int(
            self.now.month > month
            or (self.now.month == month and self.now.day > day)
        )
